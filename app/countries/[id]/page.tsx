...
// -------------------- FIXED COMPONENT --------------------
type Props = {
  params: Promise<{ id: string }> | { id: string } // ✅ allows both server & client usage
}

export default async function CountryPage({ params }: Props) {
  const resolvedParams = await Promise.resolve(params) // ✅ fixes "Promise<any>" type issue
  const countryId = Number(resolvedParams.id)
  const country = countryData[countryId]
      ...
      <section className="py-16 px-4 md:px-8">
        <div className="max-w-4xl mx-auto text-center bg-primary/5 rounded-lg p-8">
          <h2 className="text-3xl font-bold text-foreground mb-4">
            Ready to Explore {country.name} Cuisine?
          </h2>
          <p className="text-lg text-muted-foreground mb-6">
            Visit one of our partner restaurants to experience authentic {country.name} flavors
          </p>
